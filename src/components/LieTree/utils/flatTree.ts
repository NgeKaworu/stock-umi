...
export function isChecked(source: FlatTree[], target: FlatTree) {
  return source?.some(
    (v) => v?.genealogy?.join() === target?.genealogy?.join(),
  );
}

export function progenyHasChecked(
  source: FlatTree[],
  selected: FlatTree[],
  target: FlatTree,
) {
  const borther = getBorther(source, target),
    selectedBorther = getBorther(selected, target);
  return isSameFlatTree(borther, selectedBorther);
}

export function isParentNeedPick(
  source: FlatTree[],
  selected: FlatTree[],
  target: FlatTree,
) {
...
export function getBorther(source: FlatTree[], target: FlatTree) {
  return source.filter(isBorther(target));
}

export function isBorther(target: FlatTree) {
  const { genealogy } = target,
    pg = genealogy?.slice(0, -1)?.join(),
    tg = genealogy?.join();
...
export function getAbled(source: FlatTree[]) {
...
export function randomFlatTreeGen(
  depth = 3,
  breadth = 3,
  genealogy: any[] = [],
): FlatTree[] {
