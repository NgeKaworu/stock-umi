...
export default ({
  ...
  function getRelevantNode(
    ...
    while (
      ...
      !!parent &&
      parent?.children?.every((borther) => {
        const bg = borther?.genealogy!?.join();
    ...
    return ([] as Tree[])
      .concat(getProgeny(tree), tree, bubble2Root)
      .filter(Boolean);
  }
      ...
      requestIdleCallback(() => {
        ...
        for (const s of subTree) {
          curValue = uniqueRight(
            ([] as Tree[]).concat(
              curValue,
              getRelevantNode(getAble(s), root, curCheckedSet),
            ),
            (tree) => tree?.genealogy?.join(),
          );
          curCheckedSet = curValue?.reduce((acc: Set<string>, cur: Tree) => {
      ...
      const { label, genealogy, disabled } = current,
        ...
        checked = isLeaf
          ...
          : current?.children?.every((child) =>
              checkedSet.has(child?.genealogy?.join()),
            ),
        indeterminate = indeterminateSet?.has(gk);

      return (
        <div
          className={styles['check-wrap']}
          key={genealogy?.join()}
          style={style}
        >
          <Checkbox
    ...
    for (let level = 0; level <= showGroup?.length; level++) {
      ...
      columns.push(
        <Spin spinning={loading} wrapperClassName={styles.spin}>
          <div className={[styles.flex, styles.colunm].join(' ')} key={level}>
            <Space size="small">
            ...
            <div
              className={[styles.flex, styles.colunm].join(' ')}
              style={{ flex: 1 }}
            >
              {subTree?.length ? (
                <AutoSizer>
