...
export const LieTreeAsync = curry(
  (
    { request }: LieTreeAsyncParams,
    forwardProps: any,
    LieTree: React.ReactElement,
  ) => {
    ...
    async function onExpand(value?: Tree) {
      ...
      setFlatTrees((pre) =>
        uniqueRight([...pre, ...res], (tree) => tree?.genealogy?.join()),
      );
      setLoading(false);
    }
...
export default curry(
  (param: LieTreeAsyncParams, LieTree: React.ReactElement) => (
    <ChildrenRender>
      {(forwardProps) => LieTreeAsync(param, forwardProps, LieTree)}
    </ChildrenRender>
  ),
);
